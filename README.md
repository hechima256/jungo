...
純碁は、通常の囲碁から「地」の概念を取り除き、盤上の石数のみで勝敗を決めるシンプルな囲碁のバリアントです。このライブラリは、純碁のゲームロジックをTypeScriptで実装し、イミュータブルなAPIを提供します。
...
2. **石の取り**: 呼吸点がなくなった石は盤面から取り除かれます
3. **自殺手禁止**: 置いた瞬間に取られる手は打てません
4. **コウ**: 直前の局面を繰り返す手は打てません
5. **終局**: 2連続パスでゲーム終了
6. **勝敗**: 盤上の石数が多い方の勝ち（純碁ルール）
